...
def run_analysis(
    ...
    resolved_data_vendors = _resolve_data_vendor_override(selections, data_vendors)
    try:
        config = _build_run_config(selections, checkpoint, data_vendors)
    ...
    # exit_checkpoint_stream() runs in the finally below, so exceptions
    # during the stream still close the per-ticker saver and recompile a clean graph.
